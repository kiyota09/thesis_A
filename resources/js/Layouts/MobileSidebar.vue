...
import {
    Menu, X, LayoutDashboard, Package, LogOut, ChevronRight,
    CreditCard, UserPlus, Spool, ClipboardList, ChartNoAxesCombined,
    HandCoins, FileUser, Truck, Factory, Boxes, ShoppingCart,
    Warehouse, Clock, CalendarDays, History, Users, Receipt,
    HelpCircle, ShieldCheck, Building2, ClipboardCheck, FileText,
    Send, ShoppingBag, User, XCircle, Eye, Award, Archive,
    CalendarCheck, UserX, AlertCircle, UserCog2, MessageSquare,
    Navigation, Camera, Layers, DollarSign, Users as UsersIcon,
    Sparkles, Palette, Wrench, CheckCircle2, UserPen, MapPin
...
// ─── PERSISTENCE HELPERS ──────────────────────────────────────────────────────
const STORAGE_PREFIX = 'mobile_sidebar_'
...
const getRoleDropdownState = (roleKey, defaultState = false) => {
    if (roleDropdownStates.value[roleKey] === undefined) {
        roleDropdownStates.value[roleKey] = getStored(`role_${roleKey}`, defaultState)
    }
...
const getFilteredHrmChildren = () => {
    ...
    return filtered.length === 0 ? [all[0]] : filtered
}
...
const getFilteredManChildren = () => {
    ...
    if (isCEO || isManufacturingManager || isSecretaryOrGM || isManufacturingSupervisor.value) {
...
// ✅ FIXED: uses route('inv.checker.dashboard') — not the non-existent 'inv.checker'
...
const navItems = computed(() => {
    ...
    if (canAccessWorkforce() && workforceChildren.length > 0) {
        featureModules.push({
...
<template>
    <div class="md:hidden">
        ...
        <nav class="fixed top-0 left-0 right-0 z-[60] bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-b border-gray-200/50 dark:border-gray-800/50 shadow-sm h-16 flex items-center justify-between px-4 transition-colors duration-300">
            <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col">
                    <span class="text-[14px] font-black tracking-tight text-gray-900 dark:text-white uppercase leading-none">
        ...
        <!-- ─── Drawer ───────────────────────────────────────────────────────── -->
        <div v-show="isOpen"
            :class="['fixed inset-y-0 left-0 z-[55] w-[80vw] max-w-sm bg-white/90 dark:bg-gray-950/90 backdrop-blur-xl flex flex-col shadow-2xl h-full pt-16 border-r border-gray-200/50 dark:border-gray-800/50 transition-transform duration-300 ease-out', isOpen ? 'translate-x-0' : '-translate-x-full']">
            ...
            <div class="flex-1 flex flex-col overflow-y-auto px-3 py-6 custom-scrollbar">
                ...
                <nav class="space-y-1">
                    <template v-for="item in navItems" :key="item.label || item.isHeading">
                        ...
                        <!-- Heading -->
                        <div v-if="item.isHeading" class="mb-2 px-2 pt-4 first:pt-0">
                        ...
                        <div v-else-if="item.isDropdown" class="space-y-1">
                            <button @click="item.toggle"
                                :class="[item.isOpen ? 'text-blue-600 bg-white/50 dark:bg-gray-900/50' : 'text-gray-500 dark:text-gray-400', 'w-full flex items-center justify-between px-3 py-3 text-[13px] font-bold rounded-xl hover:bg-white/50 dark:hover:bg-gray-900/50 transition-all duration-300']">
                                ...
                                <ChevronRight :class="['h-4 w-4 transition-transform duration-300', item.isOpen ? 'rotate-90' : 'text-gray-400']" />
                            </button>

                            <!-- Children -->
                            <div v-show="item.isOpen" class="pl-5 space-y-0.5 mt-0.5">
                                <template v-for="subItem in item.children" :key="subItem.label">

                                    <!-- Level-2 Dropdown (e.g. Quality Checker, role dropdowns) -->
                                    <div v-if="subItem.isDropdown" class="space-y-0.5">
                                        <button @click="subItem.toggle"
                                            :class="[subItem.isOpen ? 'text-blue-600 bg-white/50 dark:bg-gray-900/50' : 'text-gray-500 dark:text-gray-400', 'w-full flex items-center justify-between px-3 py-2.5 text-[12px] font-bold rounded-xl hover:bg-white/50 dark:hover:bg-gray-900/50 transition-all duration-300']">
                                            <div class="flex items-center">
                                            ...
                                            <ChevronRight :class="['h-3.5 w-3.5 transition-transform duration-300', subItem.isOpen ? 'rotate-90' : 'text-gray-400']" />
                                        </button>
                                        <div v-show="subItem.isOpen" class="pl-8 space-y-0.5">
                                            <Link v-for="link in subItem.children" :key="link.label"
                                                :href="link.href" @click="handleNavClick"
                                                :class="[isActive(link.href) ? 'text-blue-600 font-bold' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white']"
                                                class="flex items-center gap-2 py-2 px-2 text-[12px] font-medium rounded-lg transition-colors">
                                                <component :is="link.icon" class="h-3.5 w-3.5 flex-shrink-0" />
                                                {{ link.label }}
                                            </Link>
                                        </div>
                                    </div>
                                    ...
                                    <div v-else-if="subItem.isDivider"
                                        class="text-[9px] text-gray-400 uppercase tracking-wider py-1.5 px-3 font-black">
                                        {{ subItem.label }}
                                    </div>

                                    <!-- Level-2 Direct Link -->
                                    <Link v-else :href="subItem.href" @click="handleNavClick"
                                        :class="[isActive(subItem.href) ? 'text-blue-600 bg-blue-50/80 dark:bg-blue-900/20 font-bold' : 'text-gray-500 dark:text-gray-400 hover:bg-white/40 dark:hover:bg-gray-900/40 hover:text-gray-900 dark:hover:text-white']"
                                        class="flex items-center gap-2.5 px-3 py-2.5 text-[13px] font-bold rounded-xl transition-all duration-200">
                        ...
                        <!-- Level-1 Direct Link -->
                        <Link v-else :href="item.href" @click="handleNavClick"
                            ...
                            <ChevronRight v-if="isActive(item.href)"
                                :class="isSupplier ? 'text-emerald-600/40' : 'text-blue-600/40'"
                                class="h-3.5 w-3.5" />
                        </Link>
            ...
            <div class="p-3 mt-auto flex-shrink-0 border-t border-gray-100/20 dark:border-gray-800/50">
                <div class="relative group">
                    <div class="absolute inset-0 bg-gradient-to-r from-blue-500/10 to-indigo-500/10 dark:from-blue-400/10 dark:to-indigo-400/10 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                    <div class="relative bg-white/60 dark:bg-gray-900/60 backdrop-blur-md rounded-2xl p-2.5 border border-gray-100/50 dark:border-gray-800/50 shadow-lg">
                        <div class="flex items-center gap-2.5">
                            <div class="relative">
                                ...
                                <div class="absolute -bottom-0.5 -right-0.5 h-3 w-3 bg-green-500 border-2 border-white dark:border-gray-900 rounded-full"></div>
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-[11px] font-black text-gray-900 dark:text-white truncate uppercase tracking-tighter">
                                    {{ displayName }}
                                </p>
                                ...
                                <div class="flex items-center gap-1">
                                    <ShieldCheck :class="isSupplier ? 'text-emerald-500' : 'text-blue-500'" class="h-2.5 w-2.5" />
                                    <span class="text-[8px] font-black text-gray-400 uppercase truncate">{{ displayPosition }}</span>
                                </div>
                            </div>
                        ...
                        <div v-if="isManufacturingSupervisor && supervisorRoles.length > 0"
                            ...
                            <div class="space-y-1">
                                <button v-for="role in supervisorRoles" :key="role.manufacturing_role"
                                    ...
                                    :class="[activeManufacturingRole === role.manufacturing_role ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 ring-1 ring-blue-500/30' : 'bg-gray-100/70 dark:bg-gray-800/70 text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-blue-900/20']"
                                    ...
                                    <span v-if="activeManufacturingRole === role.manufacturing_role" class="float-right text-blue-500">✓</span>
                                </button>
                            </div>
                        </div>
                    </div>
        ...
        <Teleport to="body">
            <transition name="modal-fade">
                <div v-if="showLogoutModal"
                    ...
                    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 w-full max-w-sm p-6 flex flex-col items-center text-center transform transition-all duration-300 scale-100">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-6 px-2">Are you sure you want to sign out of your account?</p>
...
.modal-fade-enter-active .bg-white, .modal-fade-leave-active .bg-white { transition: transform 0.2s ease; }
.modal-fade-enter-from   .bg-white, .modal-fade-leave-to   .bg-white   { transform: scale(0.95) translateY(10px); }
</style>