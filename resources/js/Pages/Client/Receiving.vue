                    ...
                    <table class="w-full text-left">
                        <thead class="bg-gray-50/50 dark:bg-gray-800/30 text-[10px] font-black uppercase text-gray-400 tracking-[0.15em]">
                            <tr>
                                ...
                                <th class="px-8 py-5 text-center">Delivery Status</th>
                                <th class="px-8 py-5 text-center">Expected Delivery</th>
                        ...
                        <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                            <tr v-for="order in filteredOrders" :key="order.id" class="group hover:bg-gray-50/50 transition-all">
                                ...
                                <td class="px-8 py-6 text-center">
                                    <span :class="order.delivery_status === 'delivered' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'"
                                ...
                                <td class="px-8 py-6 text-right">
                                    <button @click="markAsReceived(order)" 
                                        :disabled="order.delivery_status === 'delivered' || processing[order.id]"
                                        ...
                                        <Loader2 v-if="processing[order.id]" class="h-4 w-4 animate-spin inline mr-1" />
                                        {{ order.delivery_status === 'delivered' ? 'Received' : 'Mark as Received' }}
                                    </button>
                                </td>
                             </tr>
...
import { Truck, RefreshCw, Search, Package, Loader2 } from 'lucide-vue-next';
...
const filteredOrders = computed(() => {
    let list = props.shipments;
    if (searchTerm.value) {
        const term = searchTerm.value.toLowerCase();
        list = list.filter(o => o.po_number.toLowerCase().includes(term));
    }
    if (statusFilter.value !== 'all') {
        list = list.filter(o => o.delivery_status === statusFilter.value);
    }
    return list;
});

const inTransitCount = computed(() => props.shipments.filter(o => o.delivery_status === 'in_transit').length);
const deliveredCount = computed(() => props.shipments.filter(o => o.delivery_status === 'delivered').length);
...
const markAsReceived = async (order) => {
    ...
    processing.value[order.id] = true;
    try {
        await router.post(route('client.receiving.mark', order.id));
        showToast('success', `Order ${order.po_number} marked as received. Thank you!`);
        refreshData();
    } catch (error) {
        showToast('error', 'Failed to update delivery status.');
    } finally {
        processing.value[order.id] = false;
    }
};
</script>
