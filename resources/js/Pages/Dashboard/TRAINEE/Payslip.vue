        ...
        <div class="bg-white rounded-lg shadow-md border border-gray-200 p-6">
          ...
          <div v-else class="space-y-3 max-h-96 overflow-y-auto">
            <button v-for="payslip in props.payslips" :key="payslip.id" @click="selectPayslip(payslip.id)" :class="[
              ...
              <p class="text-sm text-gray-600 mt-1">{{ format(new Date(payslip.date), 'MMM d, yyyy') }}</p>
              <div class="flex items-center justify-between mt-2">
                <span :class="[
                  'px-2 py-1 rounded text-xs font-semibold',
                  payslip.statusBadge.color === 'success' ? 'bg-green-100 text-green-800' :
                    payslip.statusBadge.color === 'warning' ? 'bg-yellow-100 text-yellow-800' :
                      payslip.statusBadge.color === 'danger' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800',
                ]">
          ...
          <div v-if="selectedPayslip" class="space-y-6">
            ...
            <div>
              ...
              <div class="space-y-3">
                ...
                <div v-if="selectedPayslip.lateTotalDeduction > 0"
                  class="flex justify-between py-2 border-b border-gray-200 text-red-600">
                ...
                <div class="flex justify-between py-3 bg-blue-50 px-3 rounded-lg">
                  ...
                  <span class="font-bold text-lg text-indigo-600">{{ formatCurrency(selectedPayslip.grossPay) }}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="space-y-3">
                ...
                <div class="flex justify-between py-2 border-b border-gray-200">
                  ...
                  <span class="font-medium text-gray-900">{{ formatCurrency(selectedPayslip.deductions.philhealth)
                  }}</span>
                </div>
                <div class="flex justify-between py-2 border-b border-gray-200">
                  <span class="text-gray-600">PAG-IBIG Contribution</span>
                  <span class="font-medium text-gray-900">{{ formatCurrency(selectedPayslip.deductions.pagibig)
                  }}</span>
                </div>
                <div class="flex justify-between py-2 border-b border-gray-200">
                  <span class="text-gray-600">Tax Withheld</span>
                  <span class="font-medium text-gray-900">{{ formatCurrency(selectedPayslip.deductions.taxWithheld)
                  }}</span>
                </div>
                <div v-if="selectedPayslip.deductions.sssLoan > 0"
                  ...
                  <span class="font-medium text-gray-900">{{ formatCurrency(selectedPayslip.deductions.sssLoan)
                  }}</span>
                </div>
            ...
            <div :class="[
              ...
              selectedPayslip.status === 'paid' ? 'bg-green-100 text-green-800' :
                selectedPayslip.status === 'approved' ? 'bg-blue-100 text-blue-800' :
                  selectedPayslip.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800',
            ]">
